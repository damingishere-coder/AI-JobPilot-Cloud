...
本指南对应 AI-JobPilot-Cloud 当前 P9 云端能力（额度系统与基础后台）。启动后只有 Nginx 绑定宿主机端口，默认入口为：
...
| `ai-worker` | AI 匹配异步任务消费、重试与结果写回 | 无 |
...
Redis 不是唯一业务事实源。用户账号、简历元数据、求职目标、岗位和审计记录保存于 PostgreSQL；Redis Session 丢失会要求用户重新登录，但不会丢失账号数据。当前 P9 已包含插件绑定与设备管理、结构化岗位采集、AI 匹配、投递任务执行回传、免费额度与幂等流水，以及仅限管理员的基础后台。

腾讯云生产上线前的部署验收、备份恢复演练和安全检查属于 P10，尚未完成。本指南用于本地/预发布 Docker 验证，不能作为“已生产上线”或“可直接邀请真实用户”的声明。
...
./scripts/restore_postgres.sh ai-jobpilot-YYYYMMDD-HHMMSS.dump ai_jobpilot_restore_verification
```
