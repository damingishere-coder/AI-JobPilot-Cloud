...
public class MatchWorker {
    ...
    /**
     * Completes a match and settles the quota reservation in the same transaction.
     *
     * <p>Reads the persisted reservation key first, then writes the terminal match
     * state via the lease-protected SQL function. Only when {@code complete_match}
     * returns true is the quota settled: SUCCEEDED → commit, FAILED → release.
     * A null key (historical match) skips settlement; a failed settle throws so
     * the whole transaction rolls back — never a "business success but quota
     * unsettled" state.</p>
     */
    private boolean completeMatch(
        ...
        return inTenant(userId, () -> {
            String reservationKey = matchRepo.findQuotaReservationKey(userId, matchId).orElse(null);
            boolean completed = matchRepo.completeMatch(
            ...
            settleQuotaReservation(userId, matchId, reservationKey, status);
            return true;
        });
    }

    private void settleQuotaReservation(UUID userId, UUID matchId, String reservationKey, String status) {
        if (reservationKey == null || reservationKey.isBlank()) {
            // Historical match created before quota integration: nothing to settle.
            return;
        }
        if ("SUCCEEDED".equals(status)) {
            quotas.commitReservation(userId, RESOURCE_AI_ANALYSIS, reservationKey,
                    REFERENCE_JOB_MATCH, matchId, REASON_AI_ANALYSIS_COMMIT);
        } else if ("FAILED".equals(status)) {
            quotas.releaseReservation(userId, RESOURCE_AI_ANALYSIS, reservationKey,
