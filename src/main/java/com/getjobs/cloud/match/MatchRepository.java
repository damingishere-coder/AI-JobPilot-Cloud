...
public class MatchRepository {
    ...
    public MatchRecord insert(
            ...
            String status,
            String fingerprint,
            String quotaReservationKey
    ) {
        return jdbc.queryForObject(
                """
                INSERT INTO app.job_matches AS m(
                    id, user_id, job_post_id, resume_id, preference_id, status,
                    input_fingerprint, quota_reservation_key
                ) VALUES (?, ?, ?, ?, ?, CAST(? AS varchar), CAST(? AS char(64)), CAST(? AS varchar))
                RETURNING %s
                """.formatted(COLUMNS),
                this::map,
                id, userId, jobPostId, resumeId, preferenceId, status, fingerprint, quotaReservationKey
        );
    }

    /**
     * Returns the quota reservation key persisted on a match, if any.
    ...
    /**
     * Persists a fresh quota reservation key for a force-requeued match.
     * Returns false when the row does not exist for this user.
     */
    public boolean updateQuotaReservationKey(UUID userId, UUID matchId, String quotaReservationKey) {
        int updated = jdbc.update(
                """
                UPDATE app.job_matches
                SET quota_reservation_key=?
                WHERE user_id=? AND id=? AND status='PENDING'
                """,
                quotaReservationKey, userId, matchId
        );
        return updated == 1;
    }
    ...
    private MatchRecord map(ResultSet rs, int row) throws SQLException {
        return new MatchRecord(
                ...
                rs.getString("quota_reservation_key")
        );
    }
