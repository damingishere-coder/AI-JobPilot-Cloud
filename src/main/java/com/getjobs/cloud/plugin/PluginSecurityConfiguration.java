 ...
 * endpoints: the anonymous bind call, the device identity view, the heartbeat,
 * the task execution API and the job capture upload API. Everything else
 * falls through to the Web session chain, so Web endpoints can never be
 * reached with a plugin token and vice versa.
 */
@Configuration
@EnableWebSecurity
@Profile("api")
...
public class PluginSecurityConfiguration {
    ...
    CorsConfigurationSource pluginCorsConfigurationSource(PluginProperties properties) {
        ...
        source.registerCorsConfiguration("/api/plugin/heartbeat", configuration);
        source.registerCorsConfiguration("/api/plugin/tasks/**", configuration);
        source.registerCorsConfiguration("/api/plugin/jobs/**", configuration);
        return source;
    }
    ...
    SecurityFilterChain pluginSecurityFilterChain(
        ...
        http.securityMatcher(
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        .requestMatchers(HttpMethod.POST, "/api/plugin/heartbeat")
                        .hasAuthority("SCOPE_device:read")
                        .requestMatchers(HttpMethod.POST, "/api/plugin/jobs/capture")
                        .hasAuthority("SCOPE_jobs:write")
                        .requestMatchers(HttpMethod.POST, "/api/plugin/jobs/batch-capture")
                        .hasAuthority("SCOPE_jobs:write")
                        ...
                        .hasAuthority("SCOPE_tasks:write")
                        .requestMatchers(HttpMethod.POST, "/api/plugin/tasks/batch-pause")
                        .hasAuthority("SCOPE_tasks:write")
                        .anyRequest().denyAll())
