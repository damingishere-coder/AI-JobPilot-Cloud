...
public class DeliveryRepository {
    ...
    private static final String TASK_COUNT_SQL = """
            ...
            WHERE t.user_id=:userId
              ...
              AND (:filterDecision=false OR m.decision=:decision)
    ...
    private static final String TASK_LIST_SQL = """
            ...
            jp.salary_text, jp.salary_min_k, jp.salary_max_k, jp.salary_months, jp.location,
            m.id AS match_id, m.score, m.decision, m.summary,
            m.strengths::text AS strengths, m.risks::text AS risks,
    ...
    /**
     * Fixed summary SQL grouped by the persistent status; the service folds
     * the rows into the four P6 buckets. The WHERE block accepts the same
     * platform/keyword/decision filters as the list endpoint, so the summary
     * counts and the filtered list always share one filtering口径.
     */
    private static final String TASK_SUMMARY_SQL = """
            SELECT t.status AS task_status, count(*) AS task_count
            FROM app.delivery_tasks t
            JOIN app.job_posts jp ON jp.id=t.job_post_id AND jp.user_id=t.user_id
            LEFT JOIN app.job_matches m ON m.id=t.job_match_id AND m.user_id=t.user_id
            WHERE t.user_id=:userId
    ...
    /**
     * P6 match-level uniqueness: exactly one task per (user, match) forever.
     * Used to resolve repeated creates back to the original task regardless of
     * the Idempotency-Key, and to recover the winner after an insert conflict.
     */
    public Optional<TaskRecord> findByMatch(UUID userId, UUID jobMatchId) {
        try {
            return Optional.ofNullable(jdbc.queryForObject(
                    "SELECT " + TASK_COLUMNS + " FROM app.delivery_tasks t "
                            + "WHERE t.user_id=? AND t.job_match_id=?",
                    this::mapTask,
                    userId, jobMatchId
            ));
        } catch (EmptyResultDataAccessException ignored) {
        ...
        try {
            return Optional.ofNullable(jdbc.queryForObject(
                    "SELECT " + TASK_COLUMNS + " FROM app.delivery_tasks t WHERE t.user_id=? AND t.job_post_id=?"
                            + " AND t.status IN ('WAITING_CONFIRM','CONFIRMED','PULLED_BY_PLUGIN','RUNNING','PAUSED_NEED_USER')",
                    this::mapTask,
                    userId, jobPostId
            ));
        } catch (EmptyResultDataAccessException ignored) {
     ...
     * confirmation (CONFIRMED/PAUSED_NEED_USER or retryable FAILED), rolls the
     * task back to WAITING_CONFIRM and clears every execution/confirmation
     * field so the user must confirm again. Non-retryable FAILED is excluded in
     * the WHERE clause so terminal business limits cannot be bypassed by an
     * edit. Zero rows (version/status changed concurrently) return false,
     * never 500.
     */
    public boolean updateGreeting(UUID userId, UUID taskId, int expectedVersion, String greeting) {
        try {
            Integer updated = jdbc.queryForObject(
                    ...
                    SET greeting = ?,
                        status = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                           OR (status = 'FAILED' AND last_error_retryable)
                                      THEN 'WAITING_CONFIRM' ELSE status END,
                        confirmed_at = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                OR (status = 'FAILED' AND last_error_retryable)
                                           THEN NULL ELSE confirmed_at END,
                        confirmed_by = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                OR (status = 'FAILED' AND last_error_retryable)
                                           THEN NULL ELSE confirmed_by END,
                        assigned_device_id = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                       OR (status = 'FAILED' AND last_error_retryable)
                                                  THEN NULL ELSE assigned_device_id END,
                        lease_id = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                             OR (status = 'FAILED' AND last_error_retryable)
                                        THEN NULL ELSE lease_id END,
                        leased_at = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                             OR (status = 'FAILED' AND last_error_retryable)
                                        THEN NULL ELSE leased_at END,
                        lease_expires_at = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                     OR (status = 'FAILED' AND last_error_retryable)
                                                THEN NULL ELSE lease_expires_at END,
                        execution_id = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                OR (status = 'FAILED' AND last_error_retryable)
                                           THEN NULL ELSE execution_id END,
                        last_error_code = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                    OR (status = 'FAILED' AND last_error_retryable)
                                               THEN NULL ELSE last_error_code END,
                        last_error_message = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                       OR (status = 'FAILED' AND last_error_retryable)
                                                  THEN NULL ELSE last_error_message END,
                        last_error_retryable = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                                         OR (status = 'FAILED' AND last_error_retryable)
                                                    THEN NULL ELSE last_error_retryable END,
                        finished_at = CASE WHEN status IN ('CONFIRMED', 'PAUSED_NEED_USER')
                                               OR (status = 'FAILED' AND last_error_retryable)
                                          THEN NULL ELSE finished_at END,
                        version = version + 1
                    WHERE user_id = ? AND id = ? AND version = ?
                      AND (status IN ('WAITING_CONFIRM', 'CONFIRMED', 'PAUSED_NEED_USER')
                           OR (status = 'FAILED' AND last_error_retryable))
                    RETURNING version
                    """,
                    (rs, row) -> rs.getInt(1),
        ...
        try {
            Integer updated = jdbc.queryForObject(
                    ...
                    SET status = 'CONFIRMED',
                        ...
                        version = version + 1
                    WHERE user_id = ? AND id = ? AND version = ?
                      AND (status IN ('WAITING_CONFIRM', 'PAUSED_NEED_USER')
                           OR (status = 'FAILED' AND last_error_retryable))
                    RETURNING version
                    """,
                    (rs, row) -> rs.getInt(1),
    ...
    /**
     * Any non-terminal task can be skipped: the Web session releases the
     * device assignment, lease and execution attribution in the same update.
     */
        ...
        try {
            Integer updated = jdbc.queryForObject(
                    ...
                    SET status = 'SKIPPED',
                        ...
                        version = version + 1
                    WHERE user_id = ? AND id = ? AND version = ?
                      AND status IN ('WAITING_CONFIRM', 'CONFIRMED', 'PULLED_BY_PLUGIN',
                                     'RUNNING', 'PAUSED_NEED_USER', 'FAILED')
                    RETURNING version
                    """,
                    (rs, row) -> rs.getInt(1),
    ...
    /** Per-status counts for the summary endpoint; the service folds them into the P6 buckets. */
    public List<StatusCountRow> summaryCounts(
            UUID userId, String platform, String keyword, String decision
    ) {
        ...
        parameters.addValue("filterDecision", decision != null);
        parameters.addValue("decision", decision == null ? "" : decision);
        return named.query(TASK_SUMMARY_SQL, parameters, (rs, row) -> new StatusCountRow(
    ...
    // ---- plugin pending list (atomic pull) ----

    /**
     * Replay read: the tasks this device already pulled but has not started
     * yet. A repeated pull returns them without any transition, so a restarted
     * service worker can never lose a handed-out task.
     */
    public List<PendingTaskRow> findPulledForDevice(
        ...
        String platformClause = platform == null ? "" : " AND jp.platform=?";
        if (platform != null) {
            parameters.add(platform);
        }
        parameters.add(limit);
        return jdbc.query(
                "SELECT t.id AS task_id, t.version AS task_version, 'PULLED_BY_PLUGIN' AS task_status, "
                        + "jp.platform AS job_platform, jp.job_url AS job_url, "
                        + "jp.external_job_id AS external_job_id, jp.title AS job_title, "
                        + "jp.company_name AS job_company, t.greeting AS task_greeting, "
                        + "t.confirmed_at AS confirmed_at, t.confirmation_version AS confirmation_version "
                        + "FROM app.delivery_tasks t "
                        + "JOIN app.job_posts jp ON jp.id=t.job_post_id AND jp.user_id=t.user_id "
                        + "WHERE t.user_id=? AND t.status='PULLED_BY_PLUGIN' AND t.assigned_device_id=? "
                        + "AND jp.platform IN (?, ?)" + platformClause
                        + " ORDER BY t.confirmed_at ASC, t.created_at ASC LIMIT ?",
                this::mapPullRow,
                parameters.toArray()
        );
    }
    ...
    /**
     * Candidate read: the CONFIRMED tasks this device may claim (unassigned or
     * already assigned to it), matching the device capabilities. The service
     * filters these by trusted job URL before the atomic claim below.
     */
    public List<PendingTaskRow> findPullCandidatesForDevice(
            UUID userId, UUID deviceId, List<String> capabilities, String platform, int limit
    ) {
        List<Object> parameters = new ArrayList<>(List.of(userId, deviceId,
                capabilities.contains("BOSS") ? "BOSS" : "__none__",
        ...
        parameters.add(limit);
        return jdbc.query(
                "SELECT t.id AS task_id, t.version AS task_version, 'CONFIRMED' AS task_status, "
                        + "jp.platform AS job_platform, jp.job_url AS job_url, "
                        + "jp.external_job_id AS external_job_id, jp.title AS job_title, "
                        + "jp.company_name AS job_company, t.greeting AS task_greeting, "
                        + "t.confirmed_at AS confirmed_at, t.confirmation_version AS confirmation_version "
    ...
    /**
     * Atomic claim of the exact (service-validated) task ids via the SECURITY
     * DEFINER function: CONFIRMED -> PULLED_BY_PLUGIN bound to the calling
     * device with one PULLED event per claimed task; lost races are skipped.
     */
    public List<PendingTaskRow> pluginPull(UUID userId, UUID deviceId, List<UUID> taskIds) {
        if (taskIds.isEmpty()) {
            return List.of();
        }
        java.sql.Array idArray = jdbc.execute((java.sql.Connection connection) ->
                connection.createArrayOf("uuid", taskIds.toArray())
        );
        try {
            return jdbc.query(
                    """
                    SELECT * FROM app.plugin_tasks_pull(CAST(? AS uuid), CAST(? AS uuid), CAST(? AS uuid[]))
                    """,
                    this::mapPullRow,
                    userId, deviceId, idArray
            );
        } finally {
            try {
                idArray.free();
            } catch (SQLException ignored) {
                // Connection-managed arrays are released with the connection anyway.
            }
        }
    }

    private PendingTaskRow mapPullRow(ResultSet rs, int row) throws SQLException {
    ...
    /**
     * Batch pause via the SECURITY DEFINER function: every RUNNING task of the
     * calling user + device moves to PAUSED_NEED_USER with one BATCH_PAUSED
     * event each; returns the paused task ids.
     */
    public List<UUID> pluginBatchPause(UUID userId, UUID deviceId, String reason) {
        return jdbc.query(
                """
                SELECT paused_task_id FROM app.plugin_tasks_batch_pause(
                    CAST(? AS uuid), CAST(? AS uuid), CAST(? AS varchar), NULL
                )
                """,
                (rs, row) -> rs.getObject("paused_task_id", UUID.class),
                userId, deviceId, reason
        );
    }

    /** Remaining RUNNING tasks for the device after a batch pause (summary only). */
    public long countRunningForDevice(UUID userId, UUID deviceId) {
        Long count = jdbc.queryForObject(
                "SELECT count(*) FROM app.delivery_tasks "
                        + "WHERE user_id=? AND assigned_device_id=? AND status='RUNNING'",
                Long.class,
                userId, deviceId
        );
    ...
    private DeliveryModels.SalaryRef salaryRef(ResultSet rs) throws SQLException {
        Integer months = rs.getObject("salary_months", Integer.class);
        return new DeliveryModels.SalaryRef(
    ...
    private DeliveryModels.MatchRef matchRef(ResultSet rs) throws SQLException {
        ...
        return new DeliveryModels.MatchRef(
                matchId, getInt(rs, "score"), rs.getString("decision"),
                rs.getString("summary"),
                firstThree(strings(rs.getString("strengths"))),
                firstThree(strings(rs.getString("risks")))
        );
    }

    /** List rows show at most the first three strengths/risks; details show the full lists. */
    private static List<String> firstThree(List<String> values) {
        return values.size() <= 3 ? values : List.copyOf(values.subList(0, 3));
    }
    ...
    public record TaskQuery(
        ...
        /** Legacy constructor without the P6 recommendation filter. */
        public TaskQuery(
                int page, int size, List<String> statuses, String platform, String keyword,
                Instant createdFrom, Instant createdTo, TaskSort sort
        ) {
            this(page, size, statuses, platform, keyword, null, createdFrom, createdTo, sort);
        }
    ...
    /** One grouped status/count pair from the fixed summary SQL. */
