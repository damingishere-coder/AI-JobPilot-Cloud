...
export default function CloudWorkbenchPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader icon={<BiHomeAlt size={28} />} title="Cloud 工作台" subtitle="简历、求职目标和个人岗位池集中管理" actions={<Button variant="outline" size="sm" onClick={() => void load()} disabled={loading}><BiRefresh />刷新</Button>} />
      ...
      <Card><CardHeader><CardTitle>第四轮边界</CardTitle><CardDescription>岗位采集、AI 匹配和投递任务将在后续轮次开放。</CardDescription></CardHeader><CardContent><p className="text-sm leading-7 text-slate-600">当前版本专注于安全保存求职资料、验证文本提取质量，并建立严格隔离的个人岗位池。服务器不会保存招聘平台密码、Cookie 或登录会话。</p></CardContent></Card>
    </div>
  )
}
