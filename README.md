...
> **当前状态：SaaS P9 已完成，P10 的仓库内上线文档与静态配置验收已完成。** Cloud 模式已经具备用户系统、简历与求职目标、岗位上传与 AI 匹配、逐条确认的投递清单、插件执行与结果回传、免费额度限制、失败结算、额度流水和管理员后台；腾讯云实际部署、HTTPS/安全组人工验收、独立备份恢复演练和账号级删除仍未完成，请勿直接用于生产 SaaS 或邀请真实用户。
...
- 免费套餐按 UTC 自然月提供 20 次 AI 分析和 10 次投递确认；AI 终态失败释放预占，插件投递失败不返还确认额度。基础后台仅允许 `ADMIN` 查看脱敏统计和人工调整额度，本轮不接真实支付。
...
| [P9 额度与基础后台](P9_QUOTA_ADMIN.md) | 免费额度、扣减/返还、幂等流水、后台权限、脱敏与接口 |
