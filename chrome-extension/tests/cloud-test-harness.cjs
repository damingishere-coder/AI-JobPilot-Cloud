...
function fetchSuccessChain(overrides = {}) {
  ...
  return async (call) => {
    ...
    if (/\/api\/plugin\/tasks\/[^/]+\/(success|fail|pause)$/.test(url)) {
      return jsonResponse(envelope({
        id: TEST_TASK_ID,
        status: /success/.test(url) ? "SUCCEEDED" : /pause/.test(url) ? "PAUSED" : "FAILED",
...
function loadPopup(options = {}) {
  ...
  const ids = [
    "bind-section", "status-section", "api-base-select", "bind-code-input", "device-name-input",
    "bind-button", "unbind-button", "status-dot", "status-text", "device-name", "device-browser",
  ...
  return { elements, storage, fetchCalls, syncCalls, context };
}
